...
export function EmployeeCalculatorScreen({ onNavigate, language, employeeData }: EmployeeCalculatorScreenProps) {
  ...
  const calculateMonthlyPayment = () => {
    ...
    if (monthlyRate === 0) return Math.round(loanAfterDown / numberOfPayments);
    const monthlyPayment = loanAfterDown * (monthlyRate * Math.pow(1 + monthlyRate, numberOfPayments)) / (Math.pow(1 + monthlyRate, numberOfPayments) - 1);
    return Math.round(monthlyPayment);
  };

  const monthlyPayment = calculateMonthlyPayment();
  const downPaymentPercent = ((downPaymentAmount / loanAmount[0]) * 100).toFixed(1);
  const housingSupportAmount = 500000;
  const isEligibleForSupport = loanAmount[0] <= 850000;
        ...
        <div className="px-6 py-6 space-y-6 pb-28">
            ...
            <div className="relative z-10 space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex-1 bg-white rounded-2xl border border-[rgba(15,76,92,0.1)] px-4 py-3"><span className="text-lg text-[#0f4c5c]">{loanAmount[0].toLocaleString()}</span></div>
                  <span className="text-sm text-gray-600" dir="auto">{isRTL ? 'ر.س' : 'SAR'}</span>
                </div>
                <Slider value={loanAmount} onValueChange={setLoanAmount} min={100000} max={10000000} step={50000} className="w-full" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm text-[#0e1e25] mb-3 block" dir="auto">{isRTL ? 'الدفعة الأولى' : 'Down Payment'}</Label>
                  <div className="flex items-center gap-2 mb-2">
                    <Input type="number" value={downPaymentAmount} onChange={(e) => setDownPaymentAmount(Math.max(loanAmount[0] * 0.05, Math.min(loanAmount[0] * 0.50, parseInt(e.target.value) || 0)))} min={Math.floor(loanAmount[0] * 0.05)} max={Math.floor(loanAmount[0] * 0.50)} className="flex-1 bg-white border-[rgba(15,76,92,0.1)] rounded-2xl px-3 py-2 text-base focus:border-[#0f4c5c] focus:ring-1 focus:ring-[#0f4c5c]" />
                  </div>
                  <p className="text-xs text-gray-500">{downPaymentPercent}%</p>
                </div>
                <div>
                  <Label className="text-sm text-[#0e1e25] mb-3 block" dir="auto">{isRTL ? 'المدة (سنوات)' : 'Term (Years)'}</Label>
                  <Input type="number" value={termYears} onChange={(e) => setTermYears(Math.max(5, Math.min(30, parseInt(e.target.value) || 5)))} min={5} max={30} className="bg-white border-[rgba(15,76,92,0.1)] rounded-2xl px-3 py-2 text-base focus:border-[#0f4c5c] focus:ring-1 focus:ring-[#0f4c5c]" />
                  <p className="text-xs text-gray-500 mt-2">{termYears * 12} {isRTL ? 'شهر' : 'months'}</p>
                </div>
              </div>
          ...
          <Card className="p-6 bg-gradient-to-br from-[#0F4C5C] to-[#0A3540] text-white border-0 shadow-[0px_8px_24px_0px_rgba(15,76,92,0.15)] rounded-2xl">
            <div className="text-center">
              ...
              <p className="text-3xl text-white mb-1">{monthlyPayment.toLocaleString()}</p>
              <p className="text-sm text-white/80" dir="auto">{isRTL ? 'ريال سعودي' : 'Saudi Riyal'}</p>
              <div className="mt-4 pt-4 border-t border-white/20 text-xs text-white/70" dir="auto">{isRTL ? 'بناءً على معدل 3.5%' : 'Based on 3.5% rate'}</div>
            </div>
          </Card>
        </div>
      </div>
